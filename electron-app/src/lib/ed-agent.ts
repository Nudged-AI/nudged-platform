  ...
  try {
    ...
    const [{ data: vision }, { data: challenges }, { data: blockers }, { data: wiseMessages }, { data: parkedThoughts }] =
      await Promise.all([
        ...
        supabase.from('parked_thoughts').select('thought_text').eq('user_id', profile.id).limit(10),
      ]);
    ...
    const parked = (parkedThoughts ?? []).map((t: { thought_text: string }) => t.thought_text).join('; ');
