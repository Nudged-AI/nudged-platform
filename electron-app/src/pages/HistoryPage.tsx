...
import { Clock, CheckCircle, AlertCircle, Target, Layers } from 'lucide-react';
...
export default function HistoryPage({ userId }: Props) {
  ...
  useEffect(() => {
    ipc.dbSessionsList().then((rows) => { setSessions(rows); setLoading(false); });
  }, [userId]);
  ...
  const totalMinutes = sessions.filter((s) => s.status === 'completed').reduce((sum, s) => sum + s.end_minutes, 0);
              ...
              {expanded === s.id && (
                <div className="px-4 pb-4 pt-0 border-t border-gray-50">
                  <div className="flex items-start gap-2 mt-3">
                    <Layers className="w-3.5 h-3.5 text-teal-600 mt-0.5 flex-shrink-0" />
                    <div>
                      <p className="text-xs font-semibold text-gray-500 mb-1.5">Allowed Apps</p>
                      <div className="flex flex-wrap gap-1.5">
                        {s.allowed_apps.map((app) => (
                          <span key={app.appName} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-lg">{app.appName}</span>
                        ))}
                      </div>
                    </div>
                  </div>
